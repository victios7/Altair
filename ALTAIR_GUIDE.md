...
16. [HTTP Server (v1.6.5vC)](#16-http-server-v17)
17. [Routes & Handlers (v1.6.5vC)](#17-routes--handlers-v17)
18. [Middleware (v1.6.5vC)](#18-middleware-v17)
19. [Rate Limiting (v1.6.5vC)](#19-rate-limiting-v17)
20. [Health Checks (v1.6.5vC)](#20-health-checks-v17)
21. [Metrics (v1.6.5vC)](#21-metrics-v17)
22. [Graceful Shutdown (v1.6.5vC)](#22-graceful-shutdown-v17)
23. [Session Management (v1.6.5vC)](#23-session-management-v17)
24. [Configuration & Env (v1.6.5vC)](#24-configuration--env-v17)
25. [Database Pool (v1.6.5vC)](#25-database-pool-v17)
26. [Job Scheduler (v1.6.5vC)](#26-job-scheduler-v17)
...
> **v1.6.5vC:** `release` is safe to call on variables that no longer exist — it is a no-op.
...
> **v1.6.5vC Bug #7 fix:** `list.length()` with parentheses now works correctly.
...
> **v1.6.5vC Bug #3 fix:** String concatenation no longer O(n²). Long repeated concat in loops no longer causes OOM.
...
> **v1.6.5vC Bug #4 fix:** Functions can now read and write global variables. Previously, globals were inaccessible inside function bodies.

> **v1.6.5vC Bug #5 fix:** Function parameters are properly freed when the function returns, eliminating a per-call memory leak.
...
> **v1.6.5vC Bug #6 fix:** Objects now use dynamic field/method arrays (~50 bytes/instance vs ~10 KB previously). This makes creating large numbers of objects viable.
...
define numeric m = items.length()   / v1.6.5vC: both forms work
...
> **v1.6.5vC Bug #1 fix:** Sequential `try/catch` blocks no longer crash after ~34 uses. The try-depth counter is now always restored correctly — both on success and on error paths.
...
## 23. Session Management (v1.6.5vC)
...
> **Note:** Altair v1.6.5vC provides the declaration syntax and connection pooling stub. Full SQL query support requires linking your PostgreSQL/MySQL/SQLite client library in the generated C.

---

## 26. Job Scheduler (v1.6.5vC)
...
listen PORT;
    ...
    route "GET" "/";
        respond.text("TodoAPI v1.6.5vC running")
    break
break
```
...
*Altair Language Reference — v1.6.5vC — Generated by `altairc guide`*
*Compiler: altairc 1.6.5vC | License: MIT*
